...
class HHG:
    ...
    '''def w_calc(self,t): #calculation of ionisation rate w
        E = self.E_driving(t)
        n = self._Z * np.sqrt(13.6 * e / (self._I_p * E_h))
        omega_t = abs(E) / np.sqrt(2 * self._I_p) 
        C_n_sq = 2**(2 * n) / (n * gamma_func_calc(n + 1) * gamma_func_calc(n))
        return C_n_sq * self._I_p * (4 * self._I_p / omega_t)**(2 * n - 1)\
                * np.exp(-4 * self._I_p / (3 * omega_t))'''
